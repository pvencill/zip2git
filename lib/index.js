...
function unzip(archive, destDir, cb){
    temp.track();
    temp.mkdir('zip-temp', function(e, zipDir){
        ...
        unzipper.on('close', function(code){
            fs.readdir(zipDir, function(err, files){
                ...
                var srcDir = zipDir;
                if(files.length === 1){
                    fs.lstat(zipDir+'/'+files[0], function(e, stats){
                        if(stats.isDirectory()) srcDir= zipDir+'/'+files[0];
                        return mvContents(srcDir, destDir, cb);
                    })
                }
                else {
                    return mvContents(srcDir, destDir, cb);
    ...
    temp.mkdir('git-temp', function(e, dir){
        if(e) return cb(e);
            ...
            clone.on('close', function(code){
                ...
                unzip(archive, dir, function(err){
                    if(err) return cb(err);
                    return updateRepo(dir, remote, options, cb)
                })
