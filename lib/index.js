...
function unzipArchive(archive, destDir, cb) {
    //not sure if this is visible as scoped but not when I call this inside unit test
	...
	if (debugOn) 
	    {
		console.log("Going to call 7za x " + archive + " -o" + zipDir);
	    }
	//Ugh, -o/tmp/foo works, but -o /tmp/foo does not.  
	//Check chronometer; it is 2015 and not 1988
	var unzipSpawn = spawn('7za',['x',archive,'-o'+zipDir]);
	unzipSpawn.on('error', cb);
	if (debugOn) 
	    {
		unzipSpawn.stdout.on('data', function(data) {console.log('Unzip.stdout: ' + data);});
		unzipSpawn.stderr.on('data', function(data) {console.log('Unzip.stderr: ' + data);});
	    }
	unzipSpawn.on('close', function(code){
            if(code !== 0) return cb(new Error('unzip process exited with code '+code+ ' for archive '+archive));
...
//TODO: how to make this only be exported for unit tests
Zip2Git.prototype._unzipArchive = unzipArchive;
